...
namespace henson
{
   ...
   * types that require special handling.  `henson::save()` and `henson::load()` call
   * the static member functions of this class.
   *
   * The default (unspecialized) version copies
   * `sizeof(T)` bytes from `&x` to or from `bb` via
   * its `henson::BinaryBuffer::save_binary()` and `henson::BinaryBuffer::load_binary()`
   ...
   * `henson::Serialization<T>` for that type. Specializations are already provided for
  ...
  //! Saves `x` to `bb` by calling `henson::Serialization<T>::save(bb,x)`.
  template<class T>
  void                  save(BinaryBuffer& bb, const T& x)          { Serialization<T>::save(bb, x); }

  //! Loads `x` from `bb` by calling `henson::Serialization<T>::load(bb,x)`.
  template<class T>
  void                  load(BinaryBuffer& bb, T& x)                { Serialization<T>::load(bb, x); }

  //! Optimization for arrays. If `henson::Serialization` is not specialized for `T`,
  ...
  struct Serialization< MemoryBuffer >
  {
    ...
    {
      henson::save(bb, x.position);
      henson::save(bb, &x.buffer[0], x.position);
    }
  ...
  struct Serialization< std::pair<X,Y> >
  {
    ...
    {
      henson::save(bb, p.first);
      henson::save(bb, p.second);
    }

    static void         load(BinaryBuffer& bb, Pair& p)
    {
      henson::load(bb, p.first);
      henson::load(bb, p.second);
    }
  };
  ...
  struct Serialization< std::tuple<Args...> >
  {
    ...
    static
    typename std::enable_if<I < sizeof...(Args), void>::type
                        save(BinaryBuffer& bb, const Tuple& t)          { henson::save(bb, std::get<I>(t)); save<I+1>(bb, t); }
    ...
    static
    typename std::enable_if<I < sizeof...(Args), void>::type
                        load(BinaryBuffer& bb, Tuple& t)                { henson::load(bb, std::get<I>(t)); load<I+1>(bb, t); }
